...
⠀⠀sdl_graphics_ptr->window⠀=⠀SDL_CreateWindow("gbaApp",
...
⠀⠀register_keys_callback(&gba,⠀&keys_update);
⠀⠀register_get_ticks_callback(&gba,⠀&get_ticks);
⠀⠀register_delay_callback(&gba,⠀&delay);
⠀⠀register_gpu_init_callback(&gba,⠀&init_graphics);
⠀⠀register_gpu_uninit_callback(&gba,⠀&uninit_graphics);
⠀⠀register_gpu_alloc_image_buffer_callback(&gba,⠀&lock_texture);
⠀⠀register_gpu_render_callback(&gba,⠀&render);
⠀⠀register_serial_callback(&gba,⠀NULL);

⠀⠀if⠀(!init(&gba,⠀argv[1],⠀NULL,⠀bootloader_file))
⠀⠀⠀⠀{
⠀⠀⠀⠀⠀⠀return⠀2;
⠀⠀⠀⠀}
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀set_color_correction(&gba,⠀!get_color_correction(&gba));
⠀⠀⠀⠀⠀⠀⠀⠀}
#endif
⠀⠀⠀⠀}
